...
execute at @e[type=armor_stand,name=ccwg_portal,distance=..100] run setblock ~ ~-1 ~ command_block[conditional=false,facing=up]{Command:"execute as @p run function ccwg:portal_tp/floating_islands",CustomName:'{"text":"@"}',SuccessCount:0,TrackOutput:1b,UpdateLastExecution:1b,auto:0b,conditionMet:1b,powered:0b}
