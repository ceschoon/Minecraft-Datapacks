...
execute at @a[scores={drop=1..}] run summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:compass",Count:1b,tag:{display:{Lore:["\"Dimension: Floating Nether\""]}}}}
execute if entity @a[scores={drop=1..}] run scoreboard players set @a drop 0

data merge entity @e[type=item,nbt={Item:{id:"minecraft:compass"}},limit=1] {Item:{tag:{LodestoneDimension:"ccwg:floating_nether", LodestoneTracked:0b}}}
