...
execute in ccwg:floating_islands if entity @s[scores={ccwg_portal_cooldown=1}] run summon armor_stand ~ ~0.5 ~ {Invisible:1,Marker:1,CustomName:"\"ccwg_portal\"",CustomNameVisible:0}
execute in ccwg:floating_islands if entity @s[scores={ccwg_portal_cooldown=1}] run tp ~ ~0.5 ~
