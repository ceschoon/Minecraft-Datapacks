...
execute if entity @p[scores={target=1},nbt={Dimension:"ccwg:void"}] store result score @p[scores={target=1}] X run data get entity @p[scores={target=1}] Pos[0] 1
execute if entity @p[scores={target=1},nbt={Dimension:"ccwg:void"}] store result score @p[scores={target=1}] Y run data get entity @p[scores={target=1}] Pos[1] 1
execute if entity @p[scores={target=1},nbt={Dimension:"ccwg:void"}] store result score @p[scores={target=1}] Z run data get entity @p[scores={target=1}] Pos[2] 1

kill @e[type=item,nbt={Item:{id:"minecraft:compass",Count:1b},PickupDelay:40s}]
execute at @a[scores={drop=1..}] run summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:compass",Count:1b,,tag:{display:{Lore:["\"Dimension: Void\""]}}}}
execute if entity @a[scores={drop=1..}] run scoreboard players set @a drop 0

data merge entity @e[type=item,nbt={Item:{id:"minecraft:compass"}},limit=1] {Item:{tag:{LodestoneDimension:"ccwg:void", LodestoneTracked:0b}}}
