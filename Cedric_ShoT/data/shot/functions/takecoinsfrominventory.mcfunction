...
# take coins from players NOT on the platform and increment their score
...
execute as @a[scores={shot_temp=1}] run scoreboard players add @s shot_ScoreCopy 1

# take coins from players on the platform and increment common basket
scoreboard players set @a shot_temp 0
...
scoreboard players add @a[scores={shot_OnPlatform=1,shot_Common=1..}] shot_ScoreCopy 1
